...
#[command(
    name = "mcp2ar",
    about = "Auto-package npm-published MCP servers for agentregistry"
)]
struct Cli {
    #[command(subcommand)]
    command: Commands,
...
enum Commands {
    ...
    Publish {
        ...
        #[arg(
            long,
            env = "ARCTL_API_BASE_URL",
            default_value = "http://localhost:12121"
        )]
        registry_url: String,
    },
...
async fn fetch_npm(
    ...
    let resp = client
        ...
        .send()
        .await
        .map_err(|e| format!("npm fetch error: {e}"))?;
    if !resp.status().is_success() {
        return Err(format!("npm returned {} for {package}", resp.status()));
    }
    resp.json::<NpmMetadata>()
        .await
        .map_err(|e| format!("npm parse error: {e}"))
}
...
fn build_artifact(
    ...
    if !npm.keywords.is_empty() {
        meta.insert("keywords".into(), json!(npm.keywords));
    }
    if let Some(a) = &author_name {
        meta.insert("author".into(), json!(a));
    }
    if let Some(l) = &npm.license {
        meta.insert("license".into(), json!(l));
    }
    if let Some(c) = collection {
        meta.insert("collection".into(), json!(c));
    }
...
async fn registry_ping(client: &reqwest::Client, url: &str) -> bool {
    client
        .get(format!("{url}/v0/servers?limit=1"))
        .send()
        .await
...
async fn registry_publish(
    ...
    let mut req = client.post(format!("{url}/v0/servers")).json(artifact);
    if let Some(t) = &token {
...
async fn introspect(npm_package: &str, timeout_ms: u64) -> Result<Vec<Value>, String> {
    ...
    let _init_resp = tokio::time::timeout_at(deadline, reader.next_line())
        .await
    ...
    stdin
        .write_all(serde_json::to_string(&list).unwrap().as_bytes())
        .await
        .map_err(|e| format!("write error: {e}"))?;
    stdin
        .write_all(b"\n")
    ...
    let tools_line = tokio::time::timeout_at(deadline, reader.next_line())
        .await
    ...
    match cli.command {
        Commands::Generate {
            ...
            match fetch_npm(&client, &npm_package, pkg_version.as_deref()).await {
                Ok(npm) => {
                    ...
                    let artifact =
                        build_artifact(&npm, &namespace, name.as_deref(), collection.as_deref());
            ...
            match fetch_npm(&client, &npm_package, pkg_version.as_deref()).await {
                Ok(npm) => {
                    ...
                    if !registry_ping(&client, &registry_url).await {
                        eprintln!(
                            "Error: Cannot reach agentregistry at {registry_url}. Is arctl running?"
                        );
                        std::process::exit(1);
                    }

                    let reg_name = artifact
                        .get("name")
                        .and_then(|n| n.as_str())
                        .unwrap_or("unknown");
                    let ver = artifact
        ...
        Commands::Batch {
            ...
            let config: BatchConfig =
                serde_json::from_str(&raw).expect("Invalid batch config JSON");
